...
namespace ContentContent.Core
{
	public class SingletonBehaviour<T> : MonoBehaviour where T : Component
	{
		...
		public static T Instance => _instance != null ? _instance : Application.isPlaying ? Initialize() : null;
		...
		protected static T Initialize()
		{
			...
			if ((_instance = FindAnyObjectByType<T>()) != null) return _instance;

			// Create a new GameObject instance to hold the singleton component.
			var gameObject = new GameObject(typeof(T).Name + "_AutoCreated");
